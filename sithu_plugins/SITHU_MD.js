...
//══════════════════════════════════════════════════════════════════════════════════════════════════════//
  ...
  const ꜱɪᴛʜᴜ_readmore = long.repeat(4001)
  const ꜱɪᴛʜᴜ_speed = require('performance-now')
...
ꜱɪᴛʜᴜ_ᴍᴅ.Module_Exports({
 ...
 async(bot, person, text ,{isCreator }) => {
   ...
   if(text.startsWith("."))
   {
      ...
      try {
            const ꜱɪᴛʜᴜ_files = await readDirectory(text);
            files.forEach(ꜱɪᴛʜᴜ_file => { res += file + '\n'; });
   ...
   let Sithum = [];
          ...
          else Sithum.push(`┏━━⟪⟪ ${sktit} ⟫━⦿\n┃⛯ •ᴄᴏᴍᴍᴀɴᴅ• ${cmd.kingcmd}`);
          if (cmd.kingclass) Sithum.push(`┃⛯ •ᴄʟᴀss• ${cmd.kingclass}`);
          if(cmd.kingpath) Sithum.push(`┃⛯ •ꜰɪʟᴇ-ᴘᴀᴛʜ• ${cmd.kingpath}\n┗━━━━━━━━━━⦿\n${name.caption}`)
          return await person.reply(Sithum.join('\n'));

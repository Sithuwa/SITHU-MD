...
Module_Exports({
  ...
  {
            let media = await sithu.downloadAndSaveMediaMessage(citel.quoted);
            let name = await getRandom('.png')
            exec(`ffmpeg -i ${media} ${name}`, (err) => {
                let buffer = fs.readFileSync(media)
                sithu.sendMessage(citel.chat, { image: buffer }, { quoted: citel })
...
const quot = citel.msg.contextInfo.quotedMessage.viewOnceMessageV2;
  if(quot)
  {
    ...
    { console.log("Quot Entered") 
       ...
       let anu = await sithu.downloadAndSaveMediaMessage(quot.message.imageMessage)
       return sithu.sendMessage(citel.chat,{image:{url : anu},caption : cap })
    }
    if(quot.message.videoMessage) 
    {
       let cap =quot.message.videoMessage.caption;
       let anu = await sithu.downloadAndSaveMediaMessage(quot.message.videoMessage)
       return sithu.sendMessage(citel.chat,{video:{url : anu},caption : cap })
    }
...
{ console.log("ViewOnce Entered") 
    ...
    { 
      ...
      let anu = await sithu.downloadAndSaveMediaMessage(citel.quoted.message.imageMessage)
      sithu.sendMessage(citel.chat,{image:{url : anu},caption : cap })
    }
    else if(citel.quoted.message.videoMessage )
    {
      let cap =citel.quoted.message.videoMessage.caption;
      let anu = await sithu.downloadAndSaveMediaMessage(citel.quoted.message.videoMessage)
      sithu.sendMessage(citel.chat,{video:{url : anu},caption : cap })
    }
...
let media  = await getBuffer(url)
                ...
                return sithu.sendMessage(citel.chat, {sticker: buffer}, {quoted: citel });
...
if(mime =="videoMessage")
{
    ...
    return await sithu.sendMessage(   citel.chat ,  { sticker: { url: buffer }, } );
}
...
Module_Exports({
        ...
        async(sithu, citel, text) => {
          ...
          let tex2 =  textt.split(';')[1] || '_' ;
            ...
            let mee = await sithu.downloadAndSaveMediaMessage(citel.quoted)
            let bg = await TelegraPh(mee)
            let thmb =await getBuffer(`https://api.memegen.link/images/custom/${tex2}/${tex1}.png?background=${bg}`)

          if (isCheck.startsWith('p') || isCheck.startsWith('P')) { await sithu.sendMessage(citel.chat , {image : thmb , caption : name.caption })  }
          else
          {
...
Module_Exports({
        ...
        async(sithu, citel, text) => {
            ...
            let pfp;
            try {
                pfp = await sithu.profilePictureUrl(citel.quoted.sender, "image");
            } catch (e) {
                pfp = THUMB_IMAGE;
            }
            ...
            if (username.name && username.name !== undefined) {
                ...
                tname = sithu.getName(citel.quoted.sender)
            }
            let body = {
                type: "quote",
                format: "png",
            ...
            return citel.send(img,{packname:name.packname,author:'Sithum Kalhara'},"sticker")
...
if (mime =="audioMessage" || mime =="videoMessage")
{
        ...
        sithu.sendMessage(citel.chat, { audio: audio, mimetype: 'audio/mpeg' }, { quoted: citel });
...
async(sithu, citel, text) => {
    ...
    try {
        ...
        await sithu.sendMessage(citel.chat, { video: { url: media ,}, caption: `*╰┈➤ 𝙶𝙴𝙽𝙴𝚁𝙰𝚃𝙴𝙳 𝙱𝚈 ${name.botname}*`  },)

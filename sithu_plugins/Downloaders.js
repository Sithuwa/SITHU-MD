...
async(Void, citel, text) => {
if (!text) return citel.send('Please, Give me  Google Drive Url') 
...
async(sithu, person, memo) => {
    ...
    return sithu.sendMessage(person.chat, {
        audio: {
            url: ttsurl,
        },
        mimetype: "audio/mpeg",
        fileName: `ttspersonsithu.m4a`,
    }, {
        quoted: person,
    });
}
 ...
 //---------------------------------------------------------------------------
    Module_Exports({
        kingcmd: "fb",
        infocmd: "Downloads video from fb.",
        kingclass: "downloader",
        react:"🎯️",
    },
async (message, match) => {
match = match || message.reply_message.text;
    ...
    if (!tax) return citel.send(`Example: ${prefix}video Surah Fateh`);
        ...
        height: 640,
             ...
             bot.sendMessage(citel.chat, sithum, { quoted: citel })
 ...
 return await Void.sendMessage(citel.chat, buttonMessaged)
    ...
    if (!tax) return citel.send(`Example: ${prefix}audio My Babe i love Your Voice`);
        ...
        if (fileSizeInMegabytes <= dlsize) {
            ...
            await sithu.sendMessage(citel.chat, buttonMessage, { quoted: citel })
            return fs.unlinkSync(`./${randomName}`);
        } else {
            citel.reply(`File Size Bigger Then 100MB.`);
        }
    ...
    async(sithu, person, memo) => {
        ...
        let memot = "*sɪᴛʜᴜ ᴍᴅ* *ʏᴏᴜ-ᴛᴜʙᴇ sᴇᴀʀᴄʜ*\n\n *_Search Result From_* " + memo + "\n\n*✯────────────────────✯*\n\n";
        let no = 1;

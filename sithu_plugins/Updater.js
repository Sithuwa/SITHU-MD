...
async function updateHerokuApp() {
    ...
    if (commits.total === 0) { return '🧚‍♂ Your Bot is Running on Latest Version.'; } 
      ...
      try { await git.addRemote('heroku', gitUrl); } catch(e) { console.log('🧚‍♂️ Heroku remote adding error');  }
      await git.push('heroku', 'main');
      return "*_🧚‍♂️ Bot Updated SuccessFully_*\n*_Wait While Restarting_*";
    }
  }
...
Module_Exports({
        ...
        async(Void, citel, text,{ isCreator }) => {
            ...
            if (commits.total === 0) return await citel.reply(`*_🧚‍♂️ HEY_* *_${name.ownername}_* *_Your Bot Is_*\n*_Running on Latest Version_*`) 
...
if(text == 'all')
{
          citel.reply(`*_🧚‍♂️ Started Updating You Bot..._*\n*_Please Wait..._*`);
...
if(name.HEROKU_APP_NAME && name.HEROKU_API_KEY )
{
     ...
     Module_Exports({
        ...
        async(Void, citel, text,{ isCreator }) => {
                ...
                await citel.send(`*_🪀 Bot SuccessFully Updated_*`)
                process.exit(0);
       })
}
/*

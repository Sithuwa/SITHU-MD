...
const events = require('./sithu_plugins')
...
try {
    ...
    console.log("Connected to the SITHU_MD Whatsapp Bot By SITHUM KALHARA")
} catch {
...
setTimeout(() => {
	...
	global.devp = fs.readFileSync(__dirname + "/assets/Sithum_Kalhara.png");
    global.slogo = 'https://telegra.ph/file/32210876ea64c8ec80bd4.jpg'
    global.sgc = 'https://chat.whatsapp.com/GU73HsSG2kXCd8EN9xQtiF'
    global.srepo = 'https://github.com/Sithuwa/SITHU-MD'
    global.scap = '*★ᴘᴏᴡᴇʀᴇᴅ ʙʏ★ ꜱɪᴛʜᴜ ᴹᴰ* '
    global.sgen = `*╰┈➤ 𝙶𝙴𝙽𝙴𝚁𝙰𝚃𝙴𝙳 𝙱𝚈 ${name.botname}*`
    global.snam = 'sɪᴛʜᴜ ᴹᴰ'
    global.scate  = `
┏━━⟪⟪ ${mztit} ⟫━⦿       
...
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                var { body } = citel
		        const Zubi = ['94761516805','94759333625'] ;
                ...
                const blockJid = ['120363221057889216@g.us' ,...global.blockJids.split(',')]
                const allowJid = ['94759333625@s.whatsapp.net' ,...global.allowJids.split(',')]
		const allowJid = ['94766447998@s.whatsapp.net' ,...global.allowJids.split(',')]
                ...
                // if (botNumber!=='94461516805@s.whatsapp.net') return
 ...
 Void.ev.on('group-participants.update', async(anu) => {
            try {
                ...
                for (let num of participants) {
                    ...
                    catch {ppuser = 'https://telegra.ph/file/4ba85b752de56b77264c5.jpg';}
/* //------------------------------------------------------------------------------------
                    if (name.antifake != '') {
...
*/ //------------------------------------------------------------------------------------
                    ...
                    if (checkinfo) {
                        ...
                        try{ if (anu.id === '120363221057889216@g.us' && !Zubair ) return }
                        catch(e){console.log(`IsSuhail : ${Zubair} \nGroup Id : ${anu.id}\nerror Returning in SITHU_MD Group : ` , e)}
                ...
                if (connection === 'open') {
                    ...
                    for (let i = 0; i < check.length; i++) {
                        try {
                            ...
                            await fs.writeFileSync(__dirname + '/../sithu_plugins/' + check[i].id + '.js', data, "utf8");
                        } catch (error) { 
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage("94761516805@s.whatsapp.net" ,{text:"There's An Error To Forward This File"},{ quoted : quote } )
}		
...
app.listen(port, () => console.log(`SITHU-MD Server listening on port http://localhost:${port}!`));

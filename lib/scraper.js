	...
	return new Promise((resolve, reject) => {	
		...
		axios({	
			...
			axios({	
				...
				resolve({	
					status: true,	
					message: "Created By Sithum Kalhara",	
					result: result	
				})	
			}).catch(reject)	
		}).catch(reject)	
...
if (fs.existsSync('./Themes/' + Config.LANG + '.json')) {	
	...
	var json = JSON.parse(fs.readFileSync('./Themes/SITHU_MD.json'));	
}	
...
const picsithubot = todlink[Math.floor(Math.random() * todlink.length)];	
resolve(picsithubot)	
})	
 }	
...
let fname = await huh.writeFileSync(__dirname+'/../sithu_plugins/System/1.js', data, "utf8")
console.log('fname '+fname)
const command = require(__dirname+'/../sithu_plugins/System/1.js', data, "utf8");
